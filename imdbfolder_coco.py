...
def prepare_data_loaders(dataset_names, data_dir, imdb_dir, shuffle_train=True, index=None):
    ...
    for i in range(len(dataset_names)):
        ...
        img_path = data_dir
        trainloader = torch.utils.data.DataLoader(ImageFolder(data_dir, transform_train, None, index, labels_train, imgnames_train), batch_size=128, shuffle=shuffle_train, num_workers=2, pin_memory=True)
        valloader = torch.utils.data.DataLoader(ImageFolder(data_dir, transform_test, None, None, labels_val, imgnames_val), batch_size=100, shuffle=False, num_workers=2, pin_memory=True)
